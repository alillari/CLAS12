...
from regression_utils import REGRESSION_TARGET_COLUMNS
...
def compute_stats(data_root, split, low_thr, high_thr, limit_size, chunk_size):
    ...
    offsets = np.arange(n_columns, dtype=np.int64)
    ...
    for start in range(0, selected.size, chunk_size):
        ...
        update_running_stats(count, mean, m2, np.asarray(first_values, dtype=np.float64))
...
def main():
    ...
    if args.output is None:
        args.output = args.data_root.parent / "stats" / "regression_target_stats.json"
