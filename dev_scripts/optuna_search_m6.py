...
GEN_YAML = f"{REPO}/scripts/configs/_optuna_m6_trial.yaml"
CKPT_ROOT = f"{REPO}/checkpoints"
STUDY_DB = f"{REPO}/sweep_analysis/optuna_study_m6.db"
DATA_ROOT = f"{REPO}/data/mmap_v4"

FIXED_WIDTH = 1536
FIXED_DEPTH = 12
TRIAL_STEPS = 10000       # short trials for fast search
TRIAL_DATA_FRACTION = 0.50   # a moderate slice, fast but representative
...
def objective(trial):
    ...
    name = f"m6_hpsearch_trial_{trial.number}"
    block = f"""
{name}:
  <<: *default
  data_root: {DATA_ROOT}
...
"""
    ...
    cmd = [
        ...
        "--run_num=m6_hpsearch", f"--root_dir={CKPT_ROOT}",
    ]
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="clas12_optim_hparams_m6",
