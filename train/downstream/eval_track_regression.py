#!/usr/bin/env python3
"""Evaluation script for downstream track-property regression."""
import os
import sys
import argparse
import gc
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluation script for track-property regression downstream tasks"
    )
    parser.add_argument(
        "--yaml_config",
        type=str,
    ...
    parser.add_argument(
        "--usepretrain",
        action="store_true",
        help="Use frozen pretrained backbone features rather than adapter-only inputs",
    )
    parser.add_argument(
        "--pretrained_ckpt",
        default=None,
        help="Optional pretrained backbone checkpoint",
    )
    ...
    if args.usepretrain:
        ...
        if params.pretrained_ckpt is None:
            raise ValueError(
                "--usepretrain requires --pretrained_ckpt when the config has no default mapping"
            )
    else:
