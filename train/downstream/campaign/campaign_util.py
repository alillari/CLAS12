...
def collate_summary(manifest: dict[str, Any]) -> None:
    ...
    with headline_out.open("w") as headline_stream:
        for run in manifest.get("runs", []):
            ...
            if delta_theta.exists():
                with delta_theta.open(newline="") as stream:
                    for fit_row in csv.DictReader(stream):
                        delta_theta_rows.append({
                            "run_id": run["run_id"],
                            "backbone_run_id": run.get("backbone_run_id"),
                            "use_pretrained_backbone": run.get("use_pretrained_backbone", True),
                            "model_family": run.get("model_family", DEFAULT_MODEL_FAMILY),
    ...
    if delta_theta_rows:
        delta_fields = sorted({key for row in delta_theta_rows for key in row})
        with delta_theta_out.open("w", newline="") as stream:
            writer = csv.DictWriter(stream, fieldnames=delta_fields)
            writer.writeheader()
