...
class DownstreamTrainer():
    ...
    def _batch_adjusted_rand(self, inverse_valid_list, assignments, mask):
        # [FIX] was: per-sample .detach().cpu().numpy() inside the loop --
        # each call is an INDIVIDUALLY-SYNCHRONIZING GPU->CPU transfer.
        # Confirmed via profiler (profile_track_finding.py, 32-step run):
        # 85,265 cudaStreamSynchronize calls and 302,314 aten::copy_ calls,
        # dominating wall time (34.3s CPU vs only 3.7s actual GPU compute).
        # This function (called up to 3x per step across training/validation,
        # each iterating the full batch) was a primary source, alongside
        # _build_track_targets's per-sample torch.unique calls (left
        # untouched -- likely structurally necessary given each event has a
        # genuinely different number of real tracks, unlike this function).
        #
        # Fix: concatenate every sample into one tensor each, transfer ONCE
        # (a single synchronization instead of one per sample), then split
        # back into per-sample numpy arrays using known per-sample lengths.
        # Produces mathematically IDENTICAL per-sample scores and mean --
        # only how the transfer is organized changed, not the computation.
        if not inverse_valid_list:
            return 0.0

        pred_valid_list = [
            assignments[batch_idx][mask[batch_idx]]
            for batch_idx in range(len(inverse_valid_list))
        ]

        lengths = [t.numel() for t in inverse_valid_list]
        inverse_valid_flat = torch.cat(inverse_valid_list).detach().cpu().numpy()
        pred_valid_flat = torch.cat(pred_valid_list).detach().cpu().numpy()

        scores = []
        offset = 0
        for length in lengths:
            iv = inverse_valid_flat[offset:offset + length]
            pv = pred_valid_flat[offset:offset + length]
            scores.append(adjusted_rand_score(iv, pv))
            offset += length
